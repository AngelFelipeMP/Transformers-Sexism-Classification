...
if __name__ == "__main__":
    ...
    df_all = pd.concat([df_train, df_dev]).reset_index(drop=True)
    ...
    for language in config.TRANSFORMERS.keys():
        for transformer in config.TRANSFORMERS[language]:
            ...
            if language!='text':
                df = df_all.loc[(df_all['language']==language), df_all.columns]
        
            print(f'\nTransformer: {transformer} - Language/Text: {language}')
            tokenizer = AutoTokenizer.from_pretrained(transformer)

            df['tokens'] = df[language].apply(lambda x: tokenizer.tokenize(x))
